...
Ext.EventManager = function(){
    ...
    var initDocReady = function(){
        ...
        // Firefox 3.x or 4.x does not support the ready state, so simply set to complete
        if (window.define && navigator.userAgent.match(/Firefox\/(3.[0-9]+.[0-9]+|4.[0-9]+)/)){
            document.readyState = "complete"
        }

        if (document.readyState === "complete") {
            window.setTimeout(fireDocReady, 1);
        }
    };
    ...
    while(m = s.match(re)){
       ...
       if(name){
           switch(name){
               ...
               case '..': name = new Function('values', 'parent', 'return parent;'); break;
...
Ext.TabPanel = Ext.extend(Ext.Panel,  {
    ...
    onRender : function(ct, position){
        ...
        if(!this.itemTpl){
            var tt = new Ext.Template(
                 '<li class="{cls} ' + Ext.VOID_CLICK_CLASS + '" id="{id}"><a class="x-tab-strip-close"></a>',
    ...
    afterRender : function(){
        if(this.container.dom){
            var nodeList = Array.prototype.slice.call(this.container.dom.querySelectorAll('.'+Ext.VOID_CLICK_CLASS))
...
Ext.extend(Ext.menu.Menu, Ext.util.Observable, {
    ...
    render : function(){
        ...
        this.focusEl = el.createChild({
            tag: "a", cls: "x-menu-focus", href: "#", tabIndex:"-1"
        });
        if(this.focusEl.dom){
            this.focusEl.dom.addEventListener('click', function(event){
                return false
            })
        }
